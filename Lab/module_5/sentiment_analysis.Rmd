...
`sentimentr` uses the same dictionary lookup approach as the previous models, but it also takes into account *negators, amplifiers (intensifiers), de-amplifiers (downtoners), and adversative conjunctions*. 
...
sentimentr_example[, c('sentiment_dict','title_text', 'subreddit')] %>% print()
```
