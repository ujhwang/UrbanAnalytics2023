...
* CRS you will use in this class:
  ...
  * PCS: EPSG 32616  
    (WGS 84/[UTM zone 16N](https://upload.wikimedia.org/wikipedia/commons/8/8d/Utm-zones-USA.svg))
]
.pull-right[
![](img/gcs_pcs.png)
]
...
---
## Data manipulation
Mind that `group_by` + `summarize` on sf data is geospatial computation.
```{r eval=F}
system.time(
  census %>% 
    group_by(county) %>% 
    summarise(med.income = median(hhincome, na.rm = TRUE)) %>% 
    print()
)
